...
# Modelo para el request de búsqueda
...
# Initialize browser and context after imports
browser = Browser()
...
async def search_endpoint(search_request: SearchRequest):
    try:
        global agent, task
        agent = Agent(task=task, llm=llm, browser_context=context)
        task = f'search in google site:facebook.com \"{search_request.name}\", site:linkedin.com \"{search_request.name}\", site:instagram.com \"{search_request.name}\" and check person information and generate a report including all the information obtained from the user.\nName, contacts, search snippets (title, url, description) of each search performed'
        ...
        return {"message": "Profile saved to profile.md"}